 ...
 * college-onefile.js
 *
 * Single-file College Management demo:
 * - Contains both server (Express) and the frontend (served from memory)
 * - JSON datastore (data.json) created next to this file
 * - Safe refresh-token as httpOnly cookie, access token in memory (localStorage)
 * - Basic features: auth (register/login/refresh/logout), users, students/me,
 *   attendance, notices, results generator, chat, mock payments, export, backup, health
 * - Concurrent-safe writes using a write queue
 * - Basic security middleware: helmet, rate-limit, CORS with credentials, cookie-parser
 *
 * Quick run (in project directory):
 * 1) Create file with this content: college-onefile.js
 * 2) npm init -y
 * 3) npm install express bcrypt jsonwebtoken uuid cors body-parser fs-extra cookie-parser helmet express-rate-limit
 *    (Optional: npm install openai  if you want AI endpoint)
 * 4) Run:
 *    PORT=4000 JWT_SECRET="your_secret_here" node college-onefile.js
 *
 * Then open in browser: http://localhost:4000
 * Seeded admin: admin@college.test / password123
 *
 * Note: This is a demo/small-scale datastore (data.json). For production use a real DB.
 */
...
// Simple write queue to avoid concurrent writes corrupting data.json
let writeQueue = Promise.resolve();
async function queuedWriteJson(file, data, options) {
  writeQueue = writeQueue.then(() => fs.writeJson(file, data, options)).catch((e) => {
...
async function loadData() {
  try {
    ...
    if (!txt) {
      ...
      await queuedWriteJson(DATA_FILE, init, { spaces: 2 });
      return init;
    }
...
async function saveData(data) {
  return queuedWriteJson(DATA_FILE, data, { spaces: 2 });
}
...
/* Inline frontend HTML (single-file). If you prefer to edit UI, change the string below. */
const FRONTEND_HTML = `
<!doctype html>
<html lang="bn">
<head>
  ...
  <title>College Management - Single File Demo</title>
  <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-slate-50 min-h-screen">
  <div class="max-w-5xl mx-auto p-4">
    <header class="flex items-center justify-between py-4">
      <h1 class="text-xl font-bold">College Management (Single File)</h1>
    ...
    <footer class="mt-6 text-sm text-slate-500">Single-file demo UI — replace with full frontend for production.</footer>
  </div>
...
// CORS - allow credentials so cookies work with same-origin (or configure origin)
...
app.post('/api/register', async (req, res) => {
  ...
  res.json({ access, refresh: null, user: { id: user.id, email: user.email, name: user.name, role: user.role } });
});
...
app.post('/api/refresh', async (req, res) => {
  ...
  const token = req.cookies?.refreshToken || tokenFromBody;
...
app.post('/api/logout', requireAuth, async (req, res) => {
  ...
  const data = await loadData();
  if (token) {
    data.refreshTokens = data.refreshTokens.filter(rt => rt.token !== token);
    await saveData(data);
  }
...
app.post('/api/backup', requireAuth, requireRole('ADMIN'), async (req, res) => {
  ...
  await queuedWriteJson(path.join(BACKUP_DIR, filename), data, { spaces: 2 });
